...
<blockquote class="layui-elem-quote news_search">
    <div class="layui-inline">
        <div class="layui-form-mid" style="font-size: 20px">社团创建记录</div>
    </div>
    <%--<div class="layui-inline">--%>
        <%--<a class="layui-btn layui-btn-normal newsAdd_btn">社团</a>--%>
    <%--</div>--%>
    <%--<div class="layui-inline">--%>
        <%--<a class="layui-btn recommend" style="background-color:#5FB878">推荐文章</a>--%>
    <%--</div>--%>
    <%--<div class="layui-inline">--%>
        <%--<a class="layui-及btn audit_btn">审核文章</a>--%>
    <%--</div>--%>
    <%--<div class="layui-inline">--%>
...
<script>
    layui.config({
        ...
        //加载页面数据
        var newsData = '';
        $.get(/*"/layui-admin/json/newsList.json",*/"/orgList?orgFounder=${sessionScope.sessionUser.id}", function(data){
            var newArray = [];
...
//                    alert(" 😥 系统异常，请与我们的工程师小哥哥联系！");
        ...
        $("body").on("click",".news_join",function(){  //启用禁用
            var _this = $(this);
            layer.prompt({
                formType: 2,
                value: '',
...
//                alert(value); //得到value
                $.ajax({
                    type: "post",
                    url: "${ctx}/orgApply",
                    data: {"oId":_this.attr("data-id"),"uId":${sessionScope.sessionUser.id},"applyReason":value},
                    async: true,
                    dataType: "json",
                    success: function (msg) {
                        if ("OK" == msg) {
...
//                        }
                        ...
                        dataHtml +=  '<td><a class="layui-btn layui-btn-normal layui-btn-mini news_view" data-id="'+currData[i].id+'"><i class="layui-icon"></i> 查看</a>'
//                            +  '<a class="layui-btn layui-btn-danger layui-btn-mini news_join" data-id="'+currData[i].id+'"><i class="layui-icon">&#xe640;</i> 加入社团</a>'
                            +'</td>'
                            +'</tr>';
                    }
                }else{
