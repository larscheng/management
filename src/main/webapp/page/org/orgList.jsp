...
<script>
    layui.config({
        ...
        //加载页面数据
        var newsData = '';
        $.get(/*"/layui-admin/json/newsList.json",*/"/orgList", function(data){
            var newArray = [];
                ...
                if(currData.length != 0){
                    for(var i=0;i<currData.length;i++){
                        ...
                        dataHtml +=  '<a class="layui-btn layui-btn-mini news_edit" data-id="'+currData[i].id+'"><i class="iconfont icon-edit"></i> 编辑</a>'
                            +  '<a class="layui-btn layui-btn-normal layui-btn-mini news_home" data-id="'+currData[i].id+'"><i class="layui-icon"></i> 首页</a>'
